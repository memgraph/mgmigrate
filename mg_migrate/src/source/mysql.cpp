...
namespace {
  ...
  std::vector<std::pair<std::string, std::string>> ListAllTables(const MysqlClient & client) {
    std::vector<std::pair<std::string, std::string>> table_names;
    try {
      auto rows = client.session()->getSchema("information_schema")
                                 .getTable("tables")
                                 .select("table_schema", "table_name")
                                 .where("table_type = 'BASE TABLE' AND table_schema NOT IN " + kSchemaBlacklist)
                                 .execute();
      ...
      while (!row.isNull()) {
        ...
        table_names.emplace_back(row.get(0).get<std::string>(), row.get(1).get<std::string>());
        DLOG(INFO) << "Found a table '" << table_names.back().second
                   << "' in schema '" << table_names.back().first << "'";
        row = rows.fetchOne();
      }
    } catch (const mysqlx::Error & e) {
      CHECK(false) << "Failed to list all tables: " << e.what();
    }
  ...
  std::vector<std::string> ListColumnsForTable(
    ...
    DLOG(INFO) << "Listing columns for table '" << table_name
    ...
    try {
      auto rows = client.session()->getSchema("information_schema")
                                 ...
                                 .execute();
      CHECK(rows.count() > 0) << "Failed to fetch columns for table '" << table_name
                              << "' in schema '" << table_schema << "'!";
      ...
      CHECK(false) << "Failed to list columns for table '" << table_name
                 << "' in schema '" << table_schema << "'!";
    }
    return columns;
  }
      ...
      const std::string &table_name)
  {
    ...
    try {
      auto rows = client.session()->sql("SHOW KEYS FROM " + table_schema + "." + table_name +
      ...
      auto row = rows.fetchOne();
      while (!row.isNull()) {
        CHECK(row.colCount() == 15 && row.get(4).getType() == mysqlx::Value::Type::STRING)
            << "Received unexpected result while trying to list primary keys of table '"
            << table_name << "' in schema '" << table_schema << "'!";

        primary_keys.emplace_back(row.get(4).get<std::string>());
        DLOG(INFO) << "'" << primary_keys.back() << "' is part of the primary key";
        row = rows.fetchOne();
      }
    } catch (const mysqlx::Error & e) {
      CHECK(false) << "Failed to fetch primary keys for table '" << table_name
  ...
  std::vector<SchemaInfo::ForeignKey> ListAllForeignKeys(
    ...
    try {
      auto rows = client.session()->sql("SELECT"
                                      ...
                                      "WHERE constraints.constraint_schema NOT IN " + kSchemaBlacklist +
                                      "  AND child.table_schema NOT IN " + kSchemaBlacklist +
      ...
      while (!row.isNull()) {
        ...
        const auto child_table = GetTableIndex(tables, row.get(1).get<std::string>(),
                                                row.get(2).get<std::string>());
        const auto child_column = GetColumnIndex(tables[child_table].columns, row.get(3).get<std::string>());
        const auto parent_table = GetTableIndex(tables, row.get(4).get<std::string>(),
                                                row.get(5).get<std::string>());
        const auto parent_column = GetColumnIndex(tables[parent_table].columns, row.get(6).get<std::string>());
      ...
      CHECK(false) << "Failed to fetch foreign keys: " << e.what();
    }
  ...
  std::vector<SchemaInfo::ExistenceConstraint> ListAllExistenceConstraints(
    ...
    try {
      auto rows = client.session()->getSchema("information_schema")
                                 ...
                                 .where("is_nullable = 'NO' AND table_schema NOT IN " + kSchemaBlacklist)
                                 .execute();
      ...
      while (!row.isNull()) {
        ...
        for (mysqlx::col_count_t i = 0; i < row.colCount(); ++i) {
          CHECK(row.get(i).getType() == mysqlx::Value::Type::STRING)
              << "Recevied unexpected result while listing existence constraints!";
        }
        const auto table = GetTableIndex(tables, row.get(0).get<std::string>(),
                                         row.get(1).get<std::string>());
        const auto column = GetColumnIndex(tables[table].columns,
                                           row.get(2).get<std::string>());
      ...
      CHECK(false) << "Failed to list existence constraints: " << e.what();
    }
  ...
  std::vector<SchemaInfo::UniqueConstraint> ListAllUniqueConstraints(
    ...
    try {
      ...
      std::optional<std::size_t> prev_table;
      ...
      while (!row.isNull()) {
        ...
        const auto table = GetTableIndex(tables, row.get(1).get<std::string>(),
                                         row.get(2).get<std::string>());
        const auto column = GetColumnIndex(tables[table].columns, row.get(3).get<std::string>());
      ...
      CHECK(false) << "Failed to list all unique constraints: " << e.what();
    }
  ...
  mg::List ConvertToList(const mysqlx::Value &value) {
    ...
    std::transform(value.begin(), value.end(), std::back_inserter(list_values), ConvertField);
  ...
  mg::Value ConvertField(const mysqlx::Value &value) {
    ...
    switch (value.getType()) {
      ...
      case Value::Type::INT64:
        return mg::Value(value.get<std::int64_t>());
      ...
      case Value::Type::RAW:
        LOG(WARNING) << "Document and raw bytes are currently not supported";
...
std::unique_ptr<MysqlClient> MysqlClient::Connect(const MysqlClient::Params &params) {
  try {
    auto session = std::make_unique<mysqlx::Session>(
      ...
      mysqlx::SessionOption::HOST, params.host,
      mysqlx::SessionOption::DB, params.database,
...
SchemaInfo MysqlSource::GetSchemaInfo() {
  ...
  for (const auto &[table_schema, table_name] : table_names) {
    const auto columns = ListColumnsForTable(*client_, table_schema, table_name);
    ...
    tables.push_back({table_schema, table_name, std::move(columns),
                     std::move(primary_key), std::vector<std::size_t>(), false});
  }
  ...
  auto existence_constraints =
      ListAllExistenceConstraints(*client_, tables);
  auto unique_constriants =
      ListAllUniqueConstraints(*client_, tables);
...
void MysqlSource::ReadTable(
  ...
  DLOG(INFO) << "Reading data from table '" << table.name
             << "' in schema '" << table.schema << "'";

  try {
    auto rows = client_->session()->getSchema(table.schema)
                                 .getTable(table.name)
                                 .select(table.columns)
                                 .execute();
    if (rows.count() == 0) {
      LOG(WARNING) << "Table '" << table.name
                   << "' in schema '" << table.schema << "' is empty!";
      return;
    }
    auto row = rows.fetchOne();
    while (!row.isNull()) {
    ...
    CHECK(false) << "Failed to read table '" << table.name
                 << "' in schema '" << table.schema << "'!";
  }
}
